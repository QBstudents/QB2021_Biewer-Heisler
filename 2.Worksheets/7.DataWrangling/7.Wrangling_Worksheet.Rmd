...
author: 'Z620: Quantitative Biodiversity, Indiana University, Timothy Biewer-Heisler'
header-includes:
   - \usepackage{array}
output: pdf_document
...
rm(list=ls()) 
getwd() 
setwd("~/GitHub/QB2021_Biewer-Heisler/2.Worksheets/7.DataWrangling")
```
...
#print(sad_coef)
summary(com1)
a = c(0.01, 0.1, 0.2, 0.5, 0.7, 0.9, 1)
quad_areas <- seq(0.005, 0.5, 0.001)
dfs <- vector("list", length(quad_areas))
for(qa in 1: length(quad_areas)){
  comm_mat1 <- sample_quadrats(com1, n_quadrats = 10, quadrat_area = quad_areas[qa], plot = FALSE,
             method = "random", avoid_overlap = T)
  com_df <- comm_mat1$spec_dat %>%
    mutate(richness = rowSums(.[, 1:ncol(comm_mat1$spec_dat)] > 1))%>%
    mutate(area = quad_areas[qa])%>%
    select(area, richness)
  dfs[[qa]] <- com_df

}
samps_nonzero <- bind_rows(dfs)%>%
  filter(richness > 0)%>%
  mutate(log10_richness = log10(richness))%>%
  mutate(log10_area = log10(area))
regression <- lm(log10_richness ~ log10_area, samps_nonzero)
print(regression$coefficients[['log10_area']])

# data(com1)
# 
# fit <- lin_pow(data = com1, compare = TRUE, con = 1) 
# summary(fit) 

SARplot <- ggplot(samps_nonzero, aes(x = log10_area, y = log10_richness)) + geom_point() + stat_smooth(method = 'lm')
SARplot
linreg <- lm(log10_richness ~ log10_area, samps_nonzero)
print(linreg$coefficients[['log10_area']])
```
...
```{r}
comThom <- sim_thomas_community(s_pool = 25, n_sim = 1000, sad_type = "lnorm", 
        sad_coef = list("meanlog" = 2, "sdlog" = 1))
# visualize site
plot(comThom)


quad_areas2 <- seq(0.005, 0.5, 0.001)
dfs2 <- vector("list", length(quad_areas2))
for(qa2 in 1: length(quad_areas2)){
  comm_Thom1 <- sample_quadrats(comThom, n_quadrats = 10, quadrat_area = quad_areas2[qa], plot = FALSE,
             method = "random", avoid_overlap = T)
  com_df <- comm_mat1$spec_dat %>%
    mutate(richness = rowSums(.[, 1:ncol(comm_Thom1$spec_dat)] > 1))%>%
    mutate(area = quad_areas2[qa])%>%
    select(area, richness)
  dfs2[[qa]] <- com_df
...
Describe. 
For both Thomas and Poisson the Z values appeared to be very high. I am not sure how this is influenced by how the artificial communities are created. While the Thomas simulation clearly had a more random distribution with clumping of species the results in the log10 SAR seem to be the same with a nonlinear increase of species richness with area. A more elaborate simulation could possibly include factors like the environment/geography of an area, changes over time, or ways to account for meta-population structure. This would allow for a more detailed view of the effects of geographic features, a view of how populations are changing, or a more accurate view of population structure.
