...
rm(list=ls())
getwd()
setwd("~/GitHub/QB2021_Biewer-Heisler/2.Worksheets/3.RStudio")
```
...
> Answer 1: The rnorm function generates random numbers that are normally distributed. The first number gives the amount of numbers to be generated. 'mean' sets what the average of these numbers will be, and 'sd' determines what the standard deviation of the set of numbers will be.
...
> Answer 2: There are 5 rows and 10 columns.
...
> Answer 3:It seems that CHLA is not correlated with anything. The other values all seem to be correlated with each other to some extent, but with some having logarithmic distributions.
...
> Answer 4: Some of the values do decrease in correlation when using non-parametric vs parametric. One should use non-parametric on non-normal data or with large sample sizes. It doesn't look like there's evidence for false discovery rate because the corrected values equal the uncorrected values. False discovery rate is important because you can determine if you may get false positives.
...
> Answer 5: As total nitrogen increases zooplankton mass increases linearly.
...
```{r}
mesonew <-read.table("data/zoops.txt", sep = "\t", header = TRUE)
mesonew.noNUTSTANK <- mesonew[,3:11]
pairs(mesonew.noNUTSTANK)
cor4 <- cor(mesonew.noNUTSTANK)
install.packages("corrplot", repos="http://cran.rstudio.com/")
require("corrplot")
corrplot(cor4, method = "ellipse")
```
>Answer 6: It seems that the biomass of CAL, DIAP, CYCL, BOSM are all positively corelated. NAUP and CERI are positively correlated. DLUM and DIAP are also positively correlated for biomass. Some are negatively correlated, but none very strongly. 
